...
the CLEAN_NAME display-name map. The shared parity fixture only tests the rows written into it (and
pins the search settings on both sides), so a new CLEAN_NAME entry added on one side alone would
slip through — the server would silently miss the cleaned name. This reads the app's source as
TEXT (no JS runtime in the pytest suite) and pins both maps equal.
"""
