"""WHIT-559 GAPS — adversarial route seams for the "spread this bill" rule action.

The implementer's tests/lambda_api/test_rules_routes_spread.py pins the create happy path, the two
422s, the 409, the self-edit-not-a-clash, the idempotent re-POST, the spread+budgetExcluded reject,
and the non-spread path. This suite adds the seams they did NOT cover, all driven through
lambda_handler:

  * spread non-boolean -> 400 (the _validate_rule_body guard runs BEFORE any capture);
  * the 409 boundary is spread-vs-spread on the SAME category only — a different category and a
    non-spread second rule are both allowed;
  * turning spread OFF through PUT clears amount/gap/seeded end-to-end;
  * toggling spread ON on an existing plain rule captures + arms seeded False;
  * a text edit (id move) of a spread rule re-captures + re-arms through the route;
  ...
  * a multi-condition (WHIT-541) spread rule captures + round-trips, and an AND that matches no
    recurring charge is a 422;
  * spreadAmount serialises as a JSON number (float) and spreadGapDays as an int.
...
# --- the spread:true validation guard ----------------------------------------------------------

def test_spread_non_boolean_is_a_400(handler, monkeypatch):
    # FAIL-ON-REVERT for the _validate_rule_body guard: a non-bool `spread` is rejected at
    # validation, BEFORE the detector ever runs. Delete the isinstance(spread, bool) check and the
    # truthy string "yes" flows through as spread -> this stops being a 400.
    repo = FakeRuleRepo()
...
# --- the per-category 409 boundary: only spread-vs-spread on the SAME category clashes ----------

def test_a_second_spreading_rule_on_a_different_category_is_allowed(handler, monkeypatch):
...
def test_a_non_spread_second_rule_on_the_same_category_is_allowed(handler, monkeypatch):
    existing = _rule("NETFLIX", "subscriptions", spread=True,
                     spread_amount=Decimal("15.99"), spread_gap_days=31)
    repo = FakeRuleRepo(rules=[existing])
    _inject(handler, monkeypatch, repo, transactions={})
...
def test_toggling_spread_on_an_existing_plain_rule_captures_and_arms(handler, monkeypatch):
    ...
    assert repo.get_rule(rule_id)["spread_seeded"] is False
...
def test_text_edit_of_a_spread_rule_recaptures_and_rearms_through_the_route(handler, monkeypatch):
    ...
    seeded = {**repo.get_rule(old_id), "spread_seeded": True}
    ...
    assert repo.get_rule(new_id)["spread_seeded"] is False     # re-armed
...
def test_capture_is_the_detector_median_not_first_or_mean(handler, monkeypatch):
    ...
    # first = 40. The captured spreadAmount must be 43.00 -> proves it's the median.
    repo = FakeRuleRepo()
    ...
    body = json.loads(resp["body"])
    assert resp["statusCode"] == 201
    assert body["spreadAmount"] == 43.0
    assert repo.minted[0]["spread_amount"] == Decimal("43.00")


# --- multi-condition rules ---------------------------------------------------------------------

def test_multi_condition_spread_rule_captures_and_round_trips(handler, monkeypatch):
    repo = FakeRuleRepo()
    _inject(handler, monkeypatch, repo,
            transactions={SPENDING: _fixed("NETFLIX", "NETFLIX SUBSCRIPTION", "-15.99")})
    resp = handler.lambda_handler(
    ...
    body = json.loads(resp["body"])
    assert resp["statusCode"] == 201
    assert body["spread"] is True and body["spreadAmount"] == 15.99
    assert len(body["conditions"]) == 2
    assert repo.minted[0]["spread_amount"] == Decimal("15.99")
...
def test_spread_fields_serialise_as_json_number_and_int(handler, monkeypatch):
    repo = FakeRuleRepo()
    _inject(handler, monkeypatch, repo,
            transactions={SPENDING: _fixed("NETFLIX", "NETFLIX SUBSCRIPTION", "-15.99")})
    resp = handler.lambda_handler(
    ...
    assert isinstance(body["spreadGapDays"], int) and not isinstance(body["spreadGapDays"], bool)
