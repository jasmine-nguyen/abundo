...
def _top_level_binding_list(path: pathlib.Path) -> list:
    """Every top-level def/class/assignment name AS A LIST (dups preserved), so [G3] can see a
    duplicate the set form would silently collapse. Covers the annotated form too, e.g.
