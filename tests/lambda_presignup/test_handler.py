...
def _ext_event(email, trigger_source="PreSignUp_ExternalProvider", email_verified=None):
    # Same as _event but defaults to the federated ExternalProvider source, which the edge
    # cases below exercise (verified-flag gate, IdP email shapes).
    return _event(email, trigger_source, email_verified)
...
# --- adversarial fail-closed edges + the ExternalProvider verified gate ------
...
def test_non_string_email_type_is_rejected(presignup, monkeypatch):
    ...
    for bad in ([ALLOWED], {"addr": ALLOWED}, 12345, ["a", "b"]):
        with pytest.raises(Exception):
            presignup.lambda_handler(_ext_event(bad), None)


def test_whitespace_only_email_is_rejected(presignup, monkeypatch):
    monkeypatch.setenv("ALLOWED_EMAILS", ALLOWED)
    with pytest.raises(Exception):
        presignup.lambda_handler(_ext_event("   \t  "), None)
...
def test_allowlist_with_blank_and_trailing_comma_entries_still_gates(presignup, monkeypatch):
    ...
    monkeypatch.setenv("ALLOWED_EMAILS", " , ," + ALLOWED + ",,  ,")
    assert presignup.lambda_handler(_ext_event(ALLOWED, email_verified="true"), None) is not None
...
def test_pass_does_not_mutate_the_event(presignup, monkeypatch):
    ...
    monkeypatch.setenv("ALLOWED_EMAILS", ALLOWED)
    event = _ext_event(ALLOWED, email_verified="true")
...
def test_external_provider_unverified_email_is_rejected(presignup, monkeypatch):
    ...
    monkeypatch.setenv("ALLOWED_EMAILS", ALLOWED)
    with pytest.raises(Exception):
        presignup.lambda_handler(_ext_event(ALLOWED, email_verified="false"), None)
    with pytest.raises(Exception):
        presignup.lambda_handler(_ext_event("intruder@evil.com", email_verified="true"), None)
...
def test_external_provider_missing_verified_attr_is_rejected(presignup, monkeypatch):
    ...
    monkeypatch.setenv("ALLOWED_EMAILS", ALLOWED)
    with pytest.raises(Exception):
        presignup.lambda_handler(_ext_event(ALLOWED), None)
...
def test_native_signup_is_not_gated_by_verified_flag(presignup, monkeypatch):
    ...
    monkeypatch.setenv("ALLOWED_EMAILS", ALLOWED)
    event = _ext_event(ALLOWED, "PreSignUp_SignUp", email_verified="false")
    assert presignup.lambda_handler(event, None) is event
