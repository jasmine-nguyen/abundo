...
describe('SpendingDonut — a cached wedge re-entering must re-target, not keep its stale emphasis', () => {
  ...
  // is selected. Its Animated.Value is cached at +1 (map never evicts). It must dim to −1 (0.4) to
  ...
  // → opacity 1. With paintedKey in the deps the effect re-targets z to −1 → 0.4. Distinct from
  ...
  it('[A8] a previously-popped wedge returning while another is selected dims (not stuck popped)', () => {
    ...
    expect(opacityOf('z')).toBeCloseTo(0.4); // re-targeted to dim, NOT the stale popped +1
  ...
  // NOTHING is selected must go full-bright (rest), not stay stuck at 0.4. E3 only covers a
  ...
  // the dep the effect never re-runs and z keeps its cached −1 → opacity 0.4 (wrongly dimmed).
  ...
  it('[A9] a previously-dimmed wedge returning while nothing is selected goes full-bright (no over-dim)', () => {
    ...
    fireEvent.press(screen.getByTestId('donut-slice-a')); // z dims to −1 (0.4), cached
    expect(opacityOf('z')).toBeCloseTo(0.4);
    ...
    expect(opacityOf('z')).toBeCloseTo(1); // back to rest, NOT stuck dimmed at 0.4
  ...
  it('[A10] the __other__ bucket newly appearing while one is selected paints dimmed', () => {
    ...
    expect(opacityOf('__other__')).toBeCloseTo(0.4); // the new bucket dims like its peers
    expect(opacityOf('a')).toBeCloseTo(1);            // a still popped
    expect(opacityOf('b')).toBeCloseTo(0.4);         // untouched peer still dimmed
  });
});
