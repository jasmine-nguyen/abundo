...
describe('selectBudgets', () => {
  it('maps rollups to Budgets and drops non-positive targets (never divide by 0)', () => {
    ...
    expect(out).toEqual([
      { id: 'coffee', budget: 100, posted: 40, pending: 10, rollover: false, carryover: 0, spreadAdjustment: 0 },
      { id: 'food', budget: 250, posted: 60, pending: 5, rollover: false, carryover: 0, spreadAdjustment: 0 },
    ]);
...
describe('selectBudgets boundaries', () => {
  it('drops a NEGATIVE target and keeps a tiny positive one', () => {
    ...
    expect(out[0]).toEqual({ id: 'tiny', budget: 0.01, posted: 0, pending: 0, rollover: false, carryover: 0, spreadAdjustment: 0 });
  });
});
