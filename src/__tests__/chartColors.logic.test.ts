...
const SEED_SLOT_HEX: Record<string, [number, string]> = {
  eatingout: [0, '#f98f98'], travel: [1, '#0bcbd3'], utilities: [2, '#b5bb51'],
  fitness: [6, '#47c1f5'], gifts: [7, '#bf9ff8'], health: [8, '#f9927e'],
  coffee: [9, '#e8a24f'], groceries: [11, '#8ec56f'], shopping: [13, '#25cdbd'],
...
describe('chartCategoryColor — the stored slot', () => {
  ...
  it('gives each built-in the colour its server slot resolves to', () => {
    ...
    // The four that move off their id-derived colour, and the nine that do not. WHIT-415 swapped
    // coffee out of this set (its slot now resolves to the same ramp entry as its fallback) and
    // utilities in.
    const moved = Object.entries(SEED_SLOT_HEX)
      .filter(([id, [slot]]) => chartCategoryColor(id, { slot }) !== chartCategoryColor(id))
      .map(([id]) => id);
    expect(moved.sort()).toEqual(['phonenet', 'shopping', 'transport', 'utilities']);
  });
