// WHIT-308/WHIT-342 — categoryTransactions GAPS (adversarial): edges the implementer's
// categoryTransactions.logic.test.ts leaves open. The rows arrive server-scoped, so these
// exercise the client total math over given rows.
...
// (Exact-id-match on the returned rows is now a server concern — see tests/lambda_api/test_category_transactions.py.)
