// WHIT-305 — a category ENTERING the donut while another is highlighted must render dimmed (0.22)
...
describe('SpendingDonut — a category entering mid-highlight is dimmed', () => {
  ...
  it('[E1] a brand-new category appearing while one is selected renders dimmed', () => {
    ...
    expect(opacityOf('z')).toBeCloseTo(0.22); // dimmed like its peers, not full-bright
    expect(opacityOf('a')).toBeCloseTo(1);    // a still popped
    expect(opacityOf('b')).toBeCloseTo(0.22); // b untouched
  });
  ...
  // and z reads its stale cached 0 → opacity 1. With paintedKey in the deps → setValue(-1) → 0.22.
  it('[E2] a category that leaves then returns while one is selected renders dimmed (not stale-bright)', () => {
    ...
    expect(opacityOf('z')).toBeCloseTo(0.22); // re-targeted to dim, not the stale cached full-bright
    expect(opacityOf('a')).toBeCloseTo(1);    // a still popped
  });
