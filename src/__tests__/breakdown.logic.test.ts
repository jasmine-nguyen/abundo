...
describe('categoryBreakdown — parent rollup + drill-down', () => {
  ...
  it('never renders the __income__ per-source bucket as a spend row nor adds it to the total (WHIT-366)', () => {
    const s = makeState({
      categories: cats,
      breakdown: {
        coffee: spend({ posted: 20, pending: 0 }),
        // The per-source income map rides in the same object as spend; it must be skipped like
        // __earned__/__rollup__ — never a spend row, never in the hero total.
        [INCOME_KEY]: { salary: { posted: 2500, pending: 300 } } as never,
      },
    });
    const { rows, total } = categoryBreakdown(s);
    expect(rows.map((r) => r.id)).toEqual(['coffee']);
...
// ===== WHIT-226 (folded from breakdownTree.gaps.logic.test.ts) — categoryBreakdown selector:
// adversarial GAPS the implementer's breakdown.logic tests miss. Value-level tree edges: a parent
// with only zero-spend subs collapsing to a leaf / emitting nothing, a combined parent preserving
// the posted-vs-pending split + surfacing pending in spentLabel, and the synthetic "Directly in"
// row's own pct share with no pct exceeding 100. No module-level const collisions (gaps declares
// none); imports already covered by the survivor.
