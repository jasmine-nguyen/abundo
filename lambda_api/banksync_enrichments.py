...
# The value fold + text-overlap test live in the shared rule engine (WHIT-527), so this
# module, the "apply my rules" sweep, and the webhook all fold and match identically.
from rule_engine import fold as _fold, overlaps as rule_overlaps_text
...
def _to_rule(enrichment: dict) -> dict | None:
    ...
    try:
        ...
        return {
            ...
            # charge — so rule_engine refuses to act on anything but a true single-leaf rule.
...
def create_rule(field: str, operator: str, value: str, category_id: str) -> dict:
    ...
    return {
        ...
        # the shape matches _to_rule's and rule_engine can act on it.
        "conditionCount": 1,
    }
